...
export class changeHotel1660124754535 implements MigrationInterface {
  name = 'changeHotel1660124754535';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE \`spa\` DROP COLUMN \`hotel_name\``);
    await queryRunner.query(
      `ALTER TABLE \`spa\` DROP COLUMN \`hotel_description\``,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`spa\` DROP COLUMN \`spa_description\``,
    );
    await queryRunner.query(`ALTER TABLE \`spa\` DROP COLUMN \`spa_name\``);
    await queryRunner.query(
      `ALTER TABLE \`spa\` ADD \`hotel_description\` varchar(255) NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`spa\` ADD \`hotel_name\` varchar(255) NOT NULL`,
    );
    await queryRunner.query(
